...
public class Thmp extends Application<ThmpConfig> {
	...
	@Override
	public void initialize(final Bootstrap<ThmpConfig> bootstrap) {
	...
	@Override
	public void run(final ThmpConfig config, final Environment env) throws Exception {
		...
		env.jersey().register(new SongController(localSongDb, safeObjectMapper, env.metrics()));

		final ExecutorService es = env.lifecycle().executorService("worker").build();
